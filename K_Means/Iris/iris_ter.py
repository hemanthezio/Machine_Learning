...
for i in range(len(X)):
    c=int(cluster[i])
    cl['c'+str(c)].append(list(X[i]))
    
for i in range(k):
    cl['c'+str(i)]=np.array(cl['c'+str(i)])
    
for i in range(k):
    plt.scatter(cl['c'+str(i)][:,[0,2]],cl['c'+str(i)][:,[1,3]])

f=dataset.feature_names.copy()
feature=np.array(f).reshape(5,1).T

datax=(np.append(X,cluster,axis=1))
data=pd.DataFrame(datax)
data.columns=list(feature)
print(data.columns)
...
ax.scatter(X[:,[0]], X[:,[1]], X[:,[2]], X[:,[3]], cmap=plt.hot())
plt.show()'''
